...
std::u16string BiDi::bidiTransform( const std::u16string& input )
{
    ...
    std::unique_ptr<UChar[]> outputText = std::make_unique<UChar[]>( input.size() * 2 ); // Maximum output of ubidi_transform is twice the size of input according to ubidi_transform.h
    uint32_t outputLength = ubiditransform_transform(transform,
                             input.c_str(), static_cast<int32_t>( input.size() ),
                             outputText.get(), static_cast<int32_t>( input.size() ) * 2,
    ...
    if ( U_FAILURE(errorCode) ) // If the algorithm fails for any reason, fall back to non-transformed text
        return input;
